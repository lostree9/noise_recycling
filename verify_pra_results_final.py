...
# Quantitative one-scale saddle-law residuals, not correlations alone.
def scale_residual(df,xcol,ycol):
    x=df[xcol].to_numpy(float); y=df[ycol].to_numpy(float)
    scale=float((x@y)/(x@x)); pred=scale*x
    rel=(y-pred)/y
    return scale,float(np.sqrt(np.mean(rel**2))),float(np.max(np.abs(rel)))
_,focus_rms,focus_max=scale_residual(focus,'focus','scaled_amp')
_,multi_rms,multi_max=scale_residual(multi,'saddle_sum','scaled_amp')
require(focus_rms<.021 and focus_max<.033,'focusing one-scale residual')
require(multi_rms<.087 and multi_max<.163,'multiple-saddle one-scale residual')
rec('one-scale rms residual focus/multiple',f'{100*focus_rms:.3f}% / {100*multi_rms:.3f}%')

# Side-wall closure check at W/d=4,5,7.
sw=pd.read_csv(ROOT/'sidewall_width_check.csv')
for name,max_frac in [('bump_offaxis',.004),('wave_b',.0002)]:
    g=sw[sw.name==name]
    base=float(g.loc[np.isclose(g.W_over_d,5.0),'slope'].iloc[0])
    spread=float(g.slope.max()-g.slope.min())
    require(spread/abs(base)<max_frac,f'sidewall width stability {name}')
    rec(f'sidewall slope spread {name}',f'{spread:.8f} ({100*spread/abs(base):.4f}%)')
...
(ROOT/'verification_output_pra_final.txt').write_text(out)
print(out)
